...
.renderHMText <- function(alignQual) {
    ...
    alignmentQuality <- lapply(seq_along(alignQual), function(i) {
        line1 <- knitr::knit_expand(text = sprintf("### %s\n", names(alignQual)[i]))
        line2 <- knitr::knit_expand(text = sprintf("\n```{r abseq_align_qual_chunk_%s, eval=params$hasAbun, fig.width=params$width, fig.height=params$height}", alignQual[[i]]))
...
```{r abseq_quality_heatmaps_pt4, echo = FALSE, eval =(params$single && params$hasAbun)}
...
```{r abseq_pearson_correlation, eval=(params$hasDiv && !params$single), fig.width=params$width, fig.height=params$height}
...
```{r abseq_spearman_correlation, eval=(params$hasDiv && !params$single), fig.width=params$width, fig.height=params$height}
...
<!-- There are issues with ggplotly and ggdendro, ignore the interactiveness
for now. -->

### Morisita Horn

```{r abseq_morisita_horn_similarity, eval=(params$hasDiv && !params$single), fig.width=params$width, fig.height=params$height}
...
```{r abseq_bray_curtis_similarity, eval=(params$hasDiv && !params$single), fig.width=params$width, fig.height=params$height}
...
```{r abseq_jaccard_similarity, eval=(params$hasDiv && !params$single), fig.width=params$width, fig.height=params$height}
