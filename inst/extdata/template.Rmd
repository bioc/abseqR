...
params:
    ...
    rootDir: "/newhome/jiahong/sandbox/immulator_out/out/report/PCR2"
...
overviewOut <- lapply(seq_along(sampleNames), function(i) {
    ...
    paste(line1, line2, line3, line4, line5, line6, line7, line8, line9,
          line10, line11, line12, line13, collapse = "\n")
})
```
...
scatterOut <- lapply(seq_along(files), function(i) {
    ...
    line3 <- knitr::knit_expand(text = sprintf('fname <- "%s"; load(fname); if (params$interactive) { plot; } else { plot; }', files[[i]]))
    line4 <- knitr::knit_expand(text = "\n```\n")
