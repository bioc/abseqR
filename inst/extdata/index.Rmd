...
generated on `r format(Sys.time(), "%a %b %d %Y %X")`
...
if (length(samples) * 7 != (length(samples) + length(links) + length(chains) + length(rawReads) + length(annotReads) + length(prodReads)) + length(filters)) {
    stop(sprintf("Expected same argument length for singleSamples (%d), links (%d), chains (%d), rawReads (%d), annotReads (%d), prodReads (%d) and filters (%d)",
                 length(samples),
                 length(links),
                 length(chains),
                 length(rawReads),
